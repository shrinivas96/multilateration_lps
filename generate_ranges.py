...
class FieldAssets:
    def __init__(self, fieldLength:float, fieldWidth:float, initPos:np.ndarray=None) -> None:
    ...
    def alternativeRunning(self) -> None:
        """
...
if __name__ == "__main__":
    ...
    print("Distances: ", np.round(obj.rangingGenerator(), 3))
    ...
    for i in range(1, total_iterations):
        # distance_meas e= obj.rangingGenerator()		# distance from all sensors: to be estimated
        obj.whereAreYouRunning()  # update player position
        player_trajectory[:, i] = obj.getPosition()  # save new position

    # gimme that plot
    plt.figure(figsize=(10, 8))
    plt.plot(player_trajectory[0, :], player_trajectory[1, :], marker="x", linewidth=2)
